...
contract TrustForge is ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public LOW_TRUST_LIMIT = 1 ether; // Trust < 300
    uint256 public MED_TRUST_LIMIT = 5 ether; // Trust 300-600
    uint256 public HIGH_TRUST_LIMIT = 20 ether; // Trust > 600 (UPDATED)
    bool public autoLimitEnabled = true;

    uint256 public manualLowLimit = LOW_TRUST_LIMIT;
    uint256 public manualMedLimit = MED_TRUST_LIMIT;
    uint256 public manualHighLimit = HIGH_TRUST_LIMIT;
    ...
    event WalletMaturityEvaluated(
        address indexed user,
        uint256 maturityLevel,
        uint256 walletAge
    );

    event LenderDeposited(
     ...
     */
    function getWalletMaturity(
        address wallet
    ) public view returns (WalletMaturity memory) {
        ...
        )
    {
        ...
        uint256 available = stats.totalLiquidity > stats.totalActiveLoans
            ? stats.totalLiquidity - stats.totalActiveLoans
            : 0;
        uint256 utilization = stats.totalLiquidity > 0
            ? (stats.totalActiveLoans * 10000) / stats.totalLiquidity
            : 0;
    ...
    function withdrawFromPool(
        ...
        uint256 availableLiquidity = stats.totalLiquidity -
            stats.totalActiveLoans;
        ...
        trackWalletActivity
    {
        ...
        uint256 lowInterest = _calculatePoolInterestShare(
            msg.sender,
            RiskPool.LOW_RISK
        );
        uint256 medInterest = _calculatePoolInterestShare(
            msg.sender,
            RiskPool.MEDIUM_RISK
        );
        uint256 highInterest = _calculatePoolInterestShare(
            msg.sender,
            RiskPool.HIGH_RISK
        );
        ...
        if (lowInterest > 0)
            poolStats[RiskPool.LOW_RISK].totalInterestPool -= lowInterest;
        if (medInterest > 0)
            poolStats[RiskPool.MEDIUM_RISK].totalInterestPool -= medInterest;
        if (highInterest > 0)
            poolStats[RiskPool.HIGH_RISK].totalInterestPool -= highInterest;

        lender.totalInterestEarned += totalInterest;
        lender.lastClaimTime = block.timestamp;

        require(
            lendingToken.transfer(msg.sender, totalInterest),
            "Transfer failed"
        );
     ...
     */
    function _calculatePoolInterestShare(
        ...
        return
            (lenderDeposit * stats.totalInterestPool) /
            stats.totalLenderDeposits;
    }
    ...
    function requestLoan(
        ...
        if (user.defaults > 0) {
            require(
                block.timestamp >
                    user.lastDefaultTime + DEFAULT_COOLDOWN_PERIOD,
        ...
        emit WalletMaturityEvaluated(
            msg.sender,
            maturity.maturityLevel,
            maturity.age
        );

        uint256 maxLoan = _calculateBorrowingLimit(
            user.trustScore,
            maturity.maturityMultiplier
        );
        ...
        uint256 availableLiquidity = stats.totalLiquidity -
            stats.totalActiveLoans;
        uint256 requiredLiquidity = amount +
            ((amount * POOL_SAFETY_BUFFER) / 10000);
        require(
    ...
    function _issueLoan(
        ...
        uint256 interestAmount = (amount * interestRate * duration) /
            (10000 * 365 days);
    ...
    function vouchForUser(
        ...
        require(
            voucher.vouchCount < MAX_VOUCHES_PER_USER,
            "Vouch limit reached"
        ); // FIX #5
    ...
    function _penalizeVouchers(address defaulter) internal {
        ...
        for (uint i = 0; i < vouchers.length; i++) {
            ...
            emit VoucherPenalized(
                voucher,
                defaulter,
                oldTrust - voucherProfile.trustScore
            );
        }
    }
     ...
     */
    function _calculateBorrowingLimit(
        uint256 trustScore,
        uint256 maturityMultiplier
    ) internal view returns (uint256) {
        uint256 baseLimit;
        ...
        if (autoLimitEnabled) {
            // AUTO MODE
            if (trustScore < 300) baseLimit = 1 ether;
            else if (trustScore < 600) baseLimit = 5 ether;
            else baseLimit = 20 ether;
        } else {
            // MANUAL MODE
            if (trustScore < 300) baseLimit = manualLowLimit;
            else if (trustScore < 600) baseLimit = manualMedLimit;
    ...
    function _calculateInterestRate(
        ...
        if (stats.totalLiquidity > 0) {
            uint256 utilization = (stats.totalActiveLoans * 100) /
                stats.totalLiquidity;
        ...
        )
    {
        ...
        uint256 trust = profile.trustScore == 0
            ? INITIAL_TRUST_SCORE
            : profile.trustScore;
    ...
    function updateTrustParameters(
        ...
        require(
            _increasePerRepayment > 0 && _increasePerRepayment <= 100,
            "Invalid increase"
        );
        require(
            _decreaseOnDefault > 0 && _decreaseOnDefault <= 500,
            "Invalid decrease"
        );
    ...
    function updateBorrowingLimits(
        ...
        require(
            _lowTrust < _medTrust && _medTrust < _highTrust,
            "Invalid progression"
        );
